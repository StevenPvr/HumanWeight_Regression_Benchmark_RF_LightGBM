...
documentation/
  methodologie.txt            # Méthodologie détaillée
  Lightgbm_results.md         # Synthèse JSON LightGBM prête à lire
  rf_results.md               # Synthèse JSON RandomForest prête à lire
  ridge_results.md            # Synthèse JSON Ridge prête à lire
...
Tuning Optuna:
- LightGBM/RandomForest: jeu d'entraînement complet (train + val) pour optimiser les hyperparamètres sans CV.
...
- **Ridge (baseline linéaire)** :
  - MSE : 445.0 | RMSE : 21.09 | MAE : 16.81 | R² : 0.017
  - Hyperparamètres optimaux dans `results/best_ridge_params.json`
  - Documentation détaillée : `documentation/ridge_results.md`

- **Features les plus importantes** (permutation importance) : `age`, `cholesterol-mg`, `session-duration-(hours)`, `pct-maxhr`.
