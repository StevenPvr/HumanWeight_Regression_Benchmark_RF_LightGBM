...
def test_fit_label_encoders_on_train_onehot_and_ignore_unseen() -> None:
    ...
    # Encoders created per original categorical column
    assert list(encoders.keys()) == ["color", "city"]
    # Numeric column preserved
    assert np.allclose(encoded_train["numeric"].to_numpy(), train_df["numeric"].to_numpy())
    # One-hot output columns must exist for train categories only
    color_cols = [c for c in encoded_train.columns if c.startswith("color_")]
    city_cols = [c for c in encoded_train.columns if c.startswith("city_")]
    assert color_cols and city_cols

    # Unseen categories in val (e.g., color=green, city=Berlin) produce all zeros
    def all_zero_row(row: pd.Series, cols: list[str]) -> bool:
        return float(row[cols].sum()) == 0.0

    assert any(all_zero_row(r, color_cols) for _, r in encoded_val.iterrows())
    assert any(all_zero_row(r, city_cols) for _, r in encoded_val.iterrows())
