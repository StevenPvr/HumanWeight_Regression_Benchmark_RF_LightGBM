...
def test_e2e_pipeline(
    ...
    monkeypatch.setattr("src.data_cleaning.main.pd.read_csv", lambda _path: raw_df.copy())
    ...
    def _fake_save_splits(
        ...
        dfs = [train_df.assign(split="train")]
        if val_df is not None:
            dfs.append(val_df.assign(split="val"))
        dfs.append(test_df.assign(split="test"))
        combined = pd.concat(dfs)
        combined.to_csv(splits_csv, index=False)
        combined.to_parquet(splits_parquet, index=False)

    monkeypatch.setattr("src.data_preparation.main.save_splits_with_marker", _fake_save_splits)
    monkeypatch.setattr(
        "src.data_preparation.main.save_label_encoders_mappings", lambda enc, j, c: None
    )
    ...
    val_df = (
        cleaned_df.drop(index=list(train_df.index))
        .sample(n=10, random_state=123)
    ...
    # Check caplog first
    caplog_messages = [
        record.getMessage() for record in caplog.records if record.name == "src.eval.main"
    ]
    ...
    assert (
        metrics_message is not None
    ), f"Expected 'LightGBM metrics:' message not found in: {messages}"
    ...
    summary_message = next(
        (msg for msg in messages if msg.startswith("LightGBM evaluation artifact confirmed at ")),
        None,
    )
    assert summary_message is not None and str(expected_json_path) in summary_message

    # ----------------------------
    # 7) baseline.main (ridge optimize + train) - E2E style
    # ----------------------------
    from src.baseline.main import main as ridge_main

    ridge_params = tmp_path / "results" / "best_ridge_params.json"
    ridge_model_out = tmp_path / "results" / "models" / "ridge"
    ridge_scaler_out = tmp_path / "results" / "models" / "ridge_scaler.joblib"

    # Fast stubs for Ridge baseline internals
    monkeypatch.setattr(
        "src.baseline.ridge.optimize_ridge_hyperparameters",
    ...
    monkeypatch.setattr(
        "src.baseline.ridge.train_ridge_with_best_params",
        lambda parquet_path, params_json_path, target_column, random_state: (
            _DummyModel(),
            _DummyScaler(),
        ),
    )
    ...
    assert rc_opt == 0 and ridge_params.exists()
