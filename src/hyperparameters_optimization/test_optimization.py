...
def _make_mock_split(
    ...
    *,
    ...
    y = coeff_f1 * X["f1"] + coeff_f2 * X["f2"] + rng.normal(scale=noise_scale, size=n)
...
def test_optimize_lightgbm_hyperparameters_with_mocked_splits(
    ...
    def fake_loader(_path: Path) -> Tuple[pd.DataFrame, pd.DataFrame]:
...
def test_optimize_lightgbm_handles_categorical_unseen_values(
    ...
    # With OneHotEncoder, unseen categories map to all-zero vectors in their group
    def zeros_in_group(df: pd.DataFrame, prefix: str) -> bool:
        cols = [c for c in df.columns if c.startswith(prefix + "_")]
        return any(float(row[cols].sum()) == 0.0 for _, row in df.iterrows()) if cols else False

    assert any(zeros_in_group(df, "color") for df in captured_val_inputs)
    assert any(zeros_in_group(df, "city") for df in captured_val_inputs)
...
def test_optimize_random_forest_handles_categorical(
    ...
    monkeypatch.setattr("src.utils.load_splits_from_parquet", lambda _p: (train_df, test_df))
...
def _mock_cli_splits() -> Tuple[pd.DataFrame, pd.DataFrame]:
...
def test_cli_default_runs_both(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert calls["lgbm"] == 1
    assert calls["rf"] == 1
    # Saved payloads should contain best_value but no validation-specific metrics key
    assert any("best_value" in payload for payload in saved)
