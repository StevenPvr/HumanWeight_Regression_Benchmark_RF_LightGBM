...
from src.utils import ensure_numeric_columns, load_splits_from_parquet, split_features_target
...
def test_load_splits_from_parquet_roundtrip(tmp_path: Path) -> None:
    ...
    splits = load_splits_from_parquet(pq_path)
    # This test explicitly creates val splits, so we expect 3 splits
    assert len(splits) == 3, "Expected 3 splits (train, val, test)"
...
def test_ensure_numeric_columns_converts_mixed_types() -> None:
    ...
    # obj_txt cannot be coerced to numeric, so encoded as categorical codes
    assert out["obj_txt"].tolist() == [0, 1, 2]
