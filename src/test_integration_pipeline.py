...
def test_stage_preparation_main(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path, random_state_factory: Callable[[int | None], np.random.RandomState]
) -> None:
    ...
    def _save(train_df: pd.DataFrame, test_df: pd.DataFrame, csv_path: str, parquet_path: str) -> None:
        Path(out_csv).parent.mkdir(parents=True, exist_ok=True)
        pd.concat([train_df.assign(split="train"), test_df.assign(split="test")]).to_csv(out_csv, index=False)
        pd.concat([train_df.assign(split="train"), test_df.assign(split="test")]).to_parquet(out_parquet, index=False)

    monkeypatch.setattr("src.data_preparation.main.save_splits_with_marker", _save)
    monkeypatch.setattr("src.data_preparation.main.save_label_encoders_mappings", lambda e, j, c: None)
    prep_main()
    assert out_csv.exists() and out_parquet.exists()
...
def test_stage_hpo_main(
    ...
    monkeypatch.setattr(
        "src.hyperparameters_optimization.main.RESULTS_DIR", tmp_path / "results"
    )
    monkeypatch.setattr(
...
def test_stage_feature_engineering_main(
    ...
    monkeypatch.setattr("src.feature_engineering.main.load_splits_from_parquet", lambda _p: (train_df, test_df))
    ...
    def _plot_importances(df: pd.DataFrame, path: Path, top_n: int | None = None) -> None:  # noqa: ARG001
...
def test_stage_training_main(
    ...
    monkeypatch.setattr("src.training.main.load_splits_from_parquet", lambda _p: (train_df, test_df))
    ...
    rc = train_main([
        "--parquet", str(tmp_path / "data" / "dataset_splits_encoded.parquet"),
...
def test_stage_eval_main(
    ...
    monkeypatch.setitem(__import__("sys").modules, "joblib", __import__("types").SimpleNamespace(load=lambda _p: object()))
    ...
    assert any(m.startswith("LightGBM evaluation artifact confirmed at ") and str(expected_json_path) in m for m in messages)
