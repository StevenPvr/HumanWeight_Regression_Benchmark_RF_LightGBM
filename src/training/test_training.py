...
from src.utils import ensure_numeric_columns, split_features_target
...
def test_train_random_forest_with_best_params_monkeypatched(
    ...
    # Expect at least one call for train split features
    assert recorded_lengths and recorded_lengths[0] == len(train_df)
    ...
    # Basic prediction sanity on training split
    X_tr_raw, _y_tr = split_features_target(train_df, TARGET_COLUMN)
    X_tr = ensure_numeric_columns(X_tr_raw)
    preds = model.predict(X_tr)
    assert np.asarray(preds).shape == (len(train_df),)
...
def test_cli_main_trains_and_saves(
    ...
    def fake_train(
        parquet_path: Path, params_json_path: Path, *, target_column: str, random_state: int
    ):
    ...
    monkeypatch.setattr(
        "src.training.main.train_lightgbm_with_best_params", fake_train_return_model
    )
    monkeypatch.setattr("src.training.main.save_lightgbm_model", fake_save)
    monkeypatch.setattr("src.training.main.load_splits_from_parquet", lambda p: (df_train, df_test))
