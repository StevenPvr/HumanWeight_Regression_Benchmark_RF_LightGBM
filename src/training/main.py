...
from src.training.training import save_lightgbm_model, train_lightgbm_with_best_params
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            "Path to concatenated Parquet with 'split' column (train/test). "
...
def load_dataset_splits_safe(parquet_path: Path) -> tuple[pd.DataFrame | None, pd.DataFrame | None]:
    """Load dataset splits while logging any failure without aborting the CLI."""

    try:
        loaded = load_splits_from_parquet(Path(parquet_path))
        if isinstance(loaded, tuple):
            if len(loaded) == 3:
                train_df, _val_df, test_df = loaded  # type: ignore[misc]
            else:
                train_df, test_df = loaded  # type: ignore[misc]
        else:
            # Fallback: single object is unexpected; treat as missing
            raise ValueError("Unexpected splits payload")
    except Exception as exc:  # pragma: no cover - defensive logging
        LOGGER.warning("Failed to load dataset splits: %s", exc)
        return None, None

    LOGGER.info(
        "Dataset splits loaded for diagnostics (train=%d)",
        len(train_df),
    )
...
def save_lightgbm_summary(
    ...
    if validation_mse is not None:
        summary_payload["validation"] = {"mse": validation_mse}
...
def save_random_forest_summary(
    ...
    WHY: Avoid validation-test style outputs; only persist core training info.
    """
...
def run_lightgbm_stage(
    ...
    save_lightgbm_summary(args, artifact_path, train_df, None, best_params)

    return ModelStageResult(artifact_path=artifact_path, model=model, validation_mse=None)
...
def run_random_forest_stage(
    ...
    save_random_forest_summary(args, rf_out_path, train_df, rf_best)

    return ModelStageResult(artifact_path=rf_out_path, model=rf, validation_mse=None)
...
def main(argv: list[str] | None = None) -> int:
    ...
    train_df, _ = load_dataset_splits_safe(Path(args.parquet))

    lightgbm_result = run_lightgbm_stage(args, selected, train_df)
    rf_result = run_random_forest_stage(args, selected, train_df)
